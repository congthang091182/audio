...
def export_pos_to_excel(df, template_file_path, output_file_path="output_pos_data_fixed.xlsx"):
    ...
    df_cleaned = df.copy()
    string_columns = ['POS', 'CHTRINH', 'TENCT']
    ...
    for idx, pos in enumerate(pos_list):
        ...
        for row_idx, row in enumerate(pos_data.itertuples(), start=15):
            ...
            new_sheet[f'D{row_idx}'] = row.LK_CHOVAY if hasattr(row, 'LK_CHOVAY') else None
            new_sheet[f'E{row_idx}'] = row.THUNO
            new_sheet[f'F{row_idx}'] = row.LK_THUNO if hasattr(row, 'LK_THUNO') else None
            new_sheet[f'G{row_idx}'] = row.GOCXOA
            new_sheet[f'H{row_idx}'] = row.LK_GOCXOA if hasattr(row, 'LK_GOCXOA') else None
            new_sheet[f'I{row_idx}'] = row.TR_DUNO
            new_sheet[f'J{row_idx}'] = row.DUNO
            new_sheet[f'K{row_idx}'] = row.TG_DUNO if hasattr(row, 'TG_DUNO') else None
            ...
            new_sheet[f'R{row_idx}'] = row.SH_DUNO if hasattr(row, 'SH_DUNO') else None
            new_sheet[f'S{row_idx}'] = row.LKSH_CHOVAY if hasattr(row, 'LKSH_CHOVAY') else None
            new_sheet[f'T{row_idx}'] = row.BQ_DUNO if hasattr(row, 'BQ_DUNO') else None
            new_sheet[f'U{row_idx}'] = row.TEN if hasattr(row, 'TEN') else None
            new_sheet[f'V{row_idx}'] = row.NGAYBC if hasattr(row, 'NGAYBC') else None
